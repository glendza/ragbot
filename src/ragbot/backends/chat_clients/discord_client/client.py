...
class RagbotDiscordChat(discord.Client, ChatService):
    ...
    async def on_message(self, message: discord.Message) -> None:
        ...
        self._logger.debug(f"Received message: {message.content}")
        ...
        if thread:
            self._logger.info(f"Received message in thread: {thread.name}")
            ...
            async with self._queue_lock:
                self._logger.debug("Forwarding message to %d subscribers...", len(self._message_queues))
    ...
    async def messages(self) -> typing.AsyncGenerator[DiscordMessage, None]:
        ...
        async with self._queue_lock:
            ...
            self._logger.debug("Subscribed to messages from %d queues", len(self._message_queues))
    ...
    async def _start_bot_if_not_running(self) -> None:
        async with self._init_lock:
            if not self._bot_run_task:
                self._logger.info("Starting the Discord bot...")
